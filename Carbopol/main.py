...
from domain import generate_domain_points_regional, get_piv_regions,generate_boundary_points_regional, inside_L
...
def main():
    ...
    boundary = generate_boundary_points_regional()
    wall_points = boundary['wall']
    wall_normals = boundary['wall_normals']
    inlet_points = boundary['inlet']
    outlet_points = boundary['outlet']
    wall_segments = boundary['wall_segments']


    # Create enhanced domain visualization
    plt.figure(figsize=(12, 10))
    plt.scatter(all_points[:, 0], all_points[:, 1], s=1, alpha=0.3, label='Domain', color='lightblue')
...
Flow rate: {piv_reference_data['mass_flow_rate']:.6f} kg/s"""
    ...
    print(f"  Domain points: {len(all_points)}")
    ...
    print(f"  Domain bounds: x=[{all_points[:,0].min():.3f}, {all_points[:,0].max():.3f}]")
    print(f"  Domain bounds: y=[{all_points[:,1].min():.3f}, {all_points[:,1].max():.3f}]")
    ...
    trained_model = train_enhanced_staged(
        model, optimizer, all_points, inlet_points, outlet_points, 
        wall_points, wall_normals, 
        points_by_region,         # <--- region_points
        region_targets,           # <--- region_targets
        piv_reference_data        # <--- ADD THIS LINE!
    )
    ...
    visualize_results(trained_model, all_points, inside_L, wall_segments, 
