...
def plot_vector_error_heatmap(comparison_data, save_path=None, show=True):
    """
    Plot a standalone heatmap of the vector error between PIV and PINN data.

    Parameters:
    -----------
    comparison_data : dict
        Debe contener las keys 'x', 'y', 'piv_u', 'pinn_u', 'piv_v', 'pinn_v'.
    save_path : str or None
        Si es string, guarda la figura en esa ruta.
    show : bool
        Si True, muestra la figura.
    """
    # Vector error absoluto
    vec_error = np.sqrt(
        (comparison_data['piv_u'] - comparison_data['pinn_u'])**2 +
        (comparison_data['piv_v'] - comparison_data['pinn_v'])**2
    )
    tri = Triangulation(comparison_data['x'], comparison_data['y'])

    plt.figure(figsize=(8, 6))
    tcf = plt.tricontourf(tri, vec_error, levels=30, cmap='viridis')
    cbar = plt.colorbar(tcf, label='Vector Error [m/s]')
    plt.xlabel('x [m]')
    plt.ylabel('y [m]')
    plt.title('Vector Error Heatmap (|PIV - PINN|)')
    plt.axis('equal')
    plt.tight_layout()
    ...
    if save_path is not None:
        ...
        print(f"Vector error heatmap saved to: {save_path}")
    if show:
        plt.show()
    else:
        plt.close()

def plot_error_heatmaps(comparison_data, save_dir=None):
        """
        Plotea los heatmaps de error absoluto de magnitud y error vectorial total en figuras separadas.
        
        Parameters
        ----------
        comparison_data : dict
            Diccionario que contiene las claves 'x', 'y', 'piv_mag', 'pinn_mag', 'piv_u', 'piv_v', 'pinn_u', 'pinn_v'.
        save_dir : str or None
            Si se especifica, guarda las figuras en este directorio. Si es None, sólo muestra.
        """
        # 1. Heatmap de error absoluto de magnitud
        mag_error_field = np.abs(comparison_data['piv_mag'] - comparison_data['pinn_mag'])
        tri_mag = Triangulation(comparison_data['x'], comparison_data['y'])

        fig1 = plt.figure(figsize=(8, 6))
        tcf_mag = plt.tricontourf(tri_mag, mag_error_field, levels=20, cmap='hot')
        plt.colorbar(tcf_mag, label='|PIV - PINN| [m/s]')
        plt.title('Error Heatmap (Magnitude)')
        plt.xlabel('x [m]')
        plt.ylabel('y [m]')
        plt.axis('equal')
        plt.tight_layout()
        if save_dir is not None:
            plt.savefig(f"{save_dir}/heatmap_error_magnitude.png", dpi=200)
        else:
            plt.show()
        plt.close(fig1)

        # 2. Heatmap de error vectorial total
        vec_error_field = np.sqrt(
            (comparison_data['piv_u'] - comparison_data['pinn_u'])**2 +
            (comparison_data['piv_v'] - comparison_data['pinn_v'])**2
        )
        tri_vec = Triangulation(comparison_data['x'], comparison_data['y'])

        fig2 = plt.figure(figsize=(8, 6))
        tcf_vec = plt.tricontourf(tri_vec, vec_error_field, levels=20, cmap='viridis')
        plt.colorbar(tcf_vec, label='Vector Error [m/s]')
        plt.title('Vector Error Heatmap')
        plt.xlabel('x [m]')
        plt.ylabel('y [m]')
        plt.axis('equal')
        plt.tight_layout()
        if save_dir is not None:
            plt.savefig(f"{save_dir}/heatmap_error_vector.png", dpi=200)
        else:
            plt.show()
        plt.close(fig2)

def plot_vector_error_scatter(comparison_data, save_path=None, show=True):
    """
    Plot a standalone scatter heatmap of the vector error (like your image).

    Parameters:
    -----------
    comparison_data : dict
        Keys: 'x', 'y', 'piv_u', 'pinn_u', 'piv_v', 'pinn_v'
    save_path : str or None
        If not None, saves the figure to this path.
    show : bool
        If True, shows the plot. If False, closes it (for scripts).
    """
    vec_error = np.sqrt(
        (comparison_data['piv_u'] - comparison_data['pinn_u'])**2 +
        (comparison_data['piv_v'] - comparison_data['pinn_v'])**2
    ...
    cbar = plt.colorbar(sc, label='Vector error [m/s]')
    plt.xlabel('x [m]')
    plt.ylabel('y [m]')
    plt.title('Vector Error Distribution')
    plt.axis('equal')
    ...
    if save_path is not None:
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        plt.savefig(save_path, dpi=300)
        print(f"Vector error scatter plot saved to: {save_path}")
    if show:
        plt.show()
    else:
        plt.close()
...
if __name__ == "__main__":
    ...
    plot_error_heatmaps(comparator.comparison_data, save_dir="plots")  
    plot_vector_error_heatmap(comparator.comparison_data, save_path="plots/vector_error_heatmap.png")
    plot_vector_error_scatter(comparator.comparison_data, save_path="plots/vector_error_scatter.png")

# O usa save_dir=None para solo mostrar
